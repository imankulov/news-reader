...
<body>
    <div class="container">
        ...
        {% if items %}
            ...
            {% endfor %}
        {% else %}
            <div class="empty">No news items yet. Run scrape_sdk.py to fetch some.</div>
        {% endif %}
    </div>
</body>
</html>
...
def _load_items():
    ...
    for filepath in DATA_DIR.glob("*.json"):
        try:
            ...
            if saved_at:
                item["saved_at_fmt"] = datetime.fromisoformat(saved_at).strftime("%b %d, %H:%M")
            else:
                ts = filepath.stem.split("-")[0]
                try:
                    item["saved_at_fmt"] = datetime.fromtimestamp(int(ts)).strftime("%b %d, %H:%M")
                    item["saved_at"] = datetime.fromtimestamp(int(ts)).isoformat()
